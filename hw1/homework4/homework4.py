    ...
    if request.method == 'POST':
        ...
        if urlparse(url).scheme not in ALLOWED_SCHEMES:
            msg = [
                ...
                'Разрешенные схемы: {}'.format(', '.join(ALLOWED_SCHEMES)),
                'error']
        else:
            key = random_key()
            cache.add(key, url)
            short_url = 'http://localhost:8000/' + key
            msg = [
                'Исходная ссылка: {}'.format(url),
                'Ваша короткая ссылка: ', short_url]
...
def redirect_view(request, key):
    value_cache = cache.get(key)
    if value_cache:
        return redirect(value_cache)
    return redirect('/')
