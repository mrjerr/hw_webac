...
# содержащий HTTP заголовки из request.META,
# начинающиеся c 'HTTP_' плюс указанный
# в URL фрагмент (часть URL после /headers/).
#
# Пример URL: /headers/HOST
# Ответ: { "HTTP_HOST": "localhost:8000" }

def headers(request, name):
    if not name:
        return JsonResponse(
            {k: v for k, v in request.META.items() if k.startswith('HTTP')})
    return JsonResponse(
        {k: v for k, v in request.META.items() if k == 'HTTP_{}'.format(name)})
...
def redirect(request, n, count):
    if int(n) == 0:
        return HttpResponse(count, content_type="text/plain")
    if count == '' or count == '0':
        return HttpResponseRedirect('/redirect/{}/1'.format(int(n) - 1))
    return HttpResponseRedirect(
        '/redirect/{}/{}'.format(int(n) - 1, int(count) + 1))
...
urlpatterns = [
    ...
    # headers/
    # headers/HOST
    url(r'^headers/(\w*)$', headers),

    # redirect/10
    # redirect/5/5
    url(r'redirect/(\d+)/(\d*)', redirect),
